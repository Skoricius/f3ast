...
def get_spot_calibration(
    ...
    spot_dwells = [
        get_spot_dwells(t, positions[:, i], max_dwt=max_dwt)
    ...
    combined_stream = Stream(
        dwells, addressable_pixels=addressable_pixels, max_dwt=max_dwt
    )
    combined_stream.recentre()

    # save the dwell times and the positions
    positions_loc = (
        np.array(np.meshgrid(np.arange(grid[0]), np.arange(grid[1])))
        .reshape(2, n_structs)
        .T
    )
...
def export_spot_calibration(file_path, **kwargs):
    ...
    np.savetxt(
        ...
        data,
        delimiter="\t",
        header="dwellTime\tpositionX\tpositionY",
        comments="",
        fmt="%3f\t%d\t%d",
    )
