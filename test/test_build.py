...
from f3ast import (Structure, load_settings, DDModel,
                   StreamBuilder, Stream, RRLModel, HeightCorrectionModel)
...
def dd_model(structure, settings, model_parameters):
    return DDModel(
        structure,
        model_parameters["gr"], model_parameters["k"], model_parameters["sigma"],
        **settings['dd_model'])


@pytest.fixture
def rrl_model(structure, settings, model_parameters):
    return RRLModel(structure,
                    model_parameters["gr"],
...
def test_stream(dd_model, settings):
    stream_builder, _ = StreamBuilder.from_model(
        dd_model, **settings['stream_builder'])
...
def height_correction_model(structure, settings, model_parameters):
    return HeightCorrectionModel(structure,
                                 model_parameters["gr"],
                                 model_parameters["sigma"],
